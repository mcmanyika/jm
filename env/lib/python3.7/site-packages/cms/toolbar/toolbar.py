...
class CMSToolbar(BaseToolbar):
    ...
    def __init__(self, request, request_path=None, _async=False):
        ...
        for key in toolbars:
            is_current_app = toolbars[key].check_current_app(
                key, self.app_name)
    ...
    def clipboard_plugin(self):
        ...
        try:
            plugin = self.clipboard.get_plugins(
            ).select_related('placeholder')[0]
        except IndexError:
            bound_plugin = None
        else:
    ...
    def _reorder_toolbars(self):
        ...
        basic_toolbar = [
            toolbar for toolbar in toolbars if toolbar.__class__ == BasicToolbar]
    ...
    def get_or_create_menu(self, key, verbose_name=None, disabled=False, side=LEFT, position=None):
        ...
        if key in self.menus:
            ...
            return menu
        menu = Menu(verbose_name, self.csrf_token,
                    disabled=disabled, side=side)
        self.menus[key] = menu
        self.add_item(menu, position=position)
        return menu
    ...
    def add_button(self, name, url, active=False, disabled=False, extra_classes=None, extra_wrapper_classes=None,
        ...
        item.add_button(name, url, active=active,
                        disabled=disabled, extra_classes=extra_classes)
    ...
    def add_modal_button(self, name, url, active=False, disabled=False, extra_classes=None, extra_wrapper_classes=None,
        ...
        self.populate()
        item = ButtonList(extra_classes=extra_wrapper_classes, side=side)
        item.add_modal_button(name, url, active=active, disabled=disabled,
                              extra_classes=extra_classes, on_close=on_close)
        self.add_item(item, position=position)
        return item
    ...
    def _call_toolbar(self, func_name):
        with force_language(self.toolbar_language):
            first = ('cms.cms_toolbars.BasicToolbar',
    ...
    def render_with_structure(self, context, nodelist):
        ...
        with force_language(self.toolbar_language):
            ...
            render_to_string(
                'cms/toolbar/toolbar_javascript.html', flatten_context(context))
        ...
        with force_language(self.toolbar_language):
            ...
            toolbar = render_to_string(
                'cms/toolbar/toolbar_with_structure.html', flatten_context(context))
